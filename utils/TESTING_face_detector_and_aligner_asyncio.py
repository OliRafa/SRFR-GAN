...
def _detect_faces(image, path):
    ...
    detected_faces = asyncio.create_task(_mtcnn_detect_faces(image))
    #detected_faces = asyncio.wait_for(_mtcnn_detect_faces(image), 1)
    #detected_faces = asyncio.gather(_mtcnn_detect_faces(image))
    #detected_faces = detected_faces.result()
    detected_faces = loop.run_until_complete(asyncio.wait(detected_faces))
...
def detect_and_align_faces(dataset_folder, destination_folder):
    ...
    batches = ichunked(dataset_folder, 500)
