...
NUM = 240_585 + 10_730 + 194_890 + 88_970 + 90_040 + 76_000

sys.stdout = open('TEST_fda_default_more_cores_3.txt', 'a')

logging.basicConfig(
    filename='TEST_fda_default_more_cores_3_logs.txt',
    level=logging.INFO
)
logger = logging.getLogger(__name__)

@bind
async def _align_face(
        image,
        path,
        facial_landmarks,
        crop_shape=(112, 112)
...
async def _extract_center_face(image, detected_faces):
...
async def _mtcnn_detect_faces(image):
...
@bind
async def _detect_faces(image, path):
    ...
    detected_faces = await _mtcnn_detect_faces(image)
...
@bind
async def _save_image(image, folder, file_name, destination_path):
    """
    """
...
async def _read_image(file_path):
    """
    """
...
async def _preprocess_pipeline(file_path, destination_folder):
    """
    """
    print('detect_and_align_faces - PID: {}'.format(os.getpid()))

    raw_image = await _read_image(file_path)
    detected_image = await _detect_faces(raw_image)
    cropped_face = await _align_face(detected_image)
    file_path_splitted = _split_file_path(cropped_face)
    result = await _save_image(file_path_splitted, destination_folder)
    return await _log_results(result)

async def run_loop(file_path):
    print('yay')
    loop = asyncio.get_event_loop()
    haha = loop.create_task(_preprocess, file_path)
    loop.run_until_complete(asyncio.wait(haha))
    #asyncio.run(_preprocess(file_path))
...
def detect_and_align_faces(dataset_folder):
    ...
    dataset_folder = it.islice(dataset_folder, NUM, NUM + 100)
    ...
    #        #future = tuple(
    #        #    (
    #        #        loop.run_in_executor(executor, _preprocess, item)
    #        #        for item in batch
    #        #    )
    #        #)
    ...
    #        #await future
    #        for item in batch:
    #            await loop.run_in_executor(executor, _preprocess, item)
    for batch in batches:
        with mp.Pool(processes=4) as pool:
            pool.imap_unordered(run_loop, batch)
