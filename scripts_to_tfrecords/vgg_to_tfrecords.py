...
def _reduce_resolution(high_resolution_image):
    low_resolution_image = cv2.cvtColor(
            ...
            cv2.COLOR_BGR2RGB)
    high_resolution_image = cv2.cvtColor(
            high_resolution_image,
            cv2.COLOR_BGR2RGB)
    return tf.image.encode_png(low_resolution_image), tf.image.encode_png(high_resolution_image)
...
def image_example(image_string_low_resolution, image_string_high_resolution, _class_id, _sample_id):
    feature = {
